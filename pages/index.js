...
function Loader() {
  ...
  return (
    <Html center>
      <div className="title">HANG DRUM</div>
...
export default function IndexPage() {
  return (
    <>
      <Head>
        <title>HangDrum</title>
        <meta name="description" content="hang drum made with threejs and blender" />
        <meta name="author" content="Tal Hayut" />
      </Head>
      <Canvas
        shadows
        // orthographic
        camera={{ fov: 50, position: [20, 5, 0] }}
        raycaster={{ computeOffsets: ({ clientX, clientY }) => ({ offsetX: clientX, offsetY: clientY }) }}>
        <Suspense fallback={<Loader />}>
          <color attach="background" args={['#a39d97']} />
          <ambientLight intensity={1} />
          <Hangdrum />
          <Environment files="./textures/puresky.hdr" resolution={2048} background />
        </Suspense>
